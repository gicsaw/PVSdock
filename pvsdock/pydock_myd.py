...
#        run_line2 = 'sdsort -n -fSCORE %s.sd' % (docking_prefix)
    ...
    def simulation_process(self, idx, mol_id, smi, pid):
        ...
        if self.neutralize or (self.pH is not None):
            smi_p = ligandtools.ligand_preparation(smi, self.neutralize,
                                                   self.pH)
        else:
            smi_p = smi
        ...
        if self.dp == 'vina':
            e = ligandtools.gen_3d(smi_p, ligand_pdb_file, mol_id=mol_id,
                                   timeout=self.timeout_gen3d)
            if e is not None:
                e2 = ligandtools.gen_3d(smi_p, ligand_pdb_file, mol_id=mol_id,
                                        timeout=self.timeout_gen3d)
                if e2 is not None:
            ...
            e = ligandtools.pdb_to_pdbqt(ligand_pdb_file, ligand_pdbqt_file)
            if e is not None:
            ...
            e = ligandtools.gen_3d(smi_p, ligand_sdf_file, mol_id=mol_id,
                                   timeout=self.timeout_gen3d)
            if e is not None:
                e2 = ligandtools.gen_3d(smi_p, ligand_sdf_file, mol_id=mol_id,
                                        timeout=self.timeout_gen3d)
                if e2 is not None:
                    print(e2, 'gen_3d', idx, mol_id, smi_p, flush=True)
                    docking_score = np.array([99.999], dtype=np.float32)
        ...
        if self.output_save or self.rescoring or self.use_my_module:
            if self.dp == 'vina':
                ...
                ligandtools.obabel_rewrite(docking_sdf_file,
